...
TMPPATH=/tmp/NssWidget-install
FILEPATH=/tmp/NssWidget-main.tar.gz

echo "Starting NssWidget installation..."

# Determine plugin path based on architecture
if [ ! -d /usr/lib64 ]; then
    PLUGINPATH=/usr/lib/enigma2/python/Plugins/Extensions/Aglare
else
    PLUGINPATH=/usr/lib64/enigma2/python/Plugins/Extensions/Aglare
fi
...
cleanup() {
    ...
    [ -d "/tmp/NssWidget-main" ] && rm -rf "/tmp/NssWidget-main"
}
...
echo "⬇️ Downloading NssWidget..."
wget --no-check-certificate 'https://github.com/Belfagor2005/NssWidget/archive/refs/heads/main.tar.gz' -O "$FILEPATH"
if [ $? -ne 0 ]; then
    echo "❌ Failed to download NssWidget package!"
    cleanup
    exit 1
fi
...
if [ $? -ne 0 ]; then
    echo "❌ Failed to extract NssWidget package!"
    cleanup
    exit 1
fi
...
if [ -d "$TMPPATH/NssWidget-main/usr/lib/enigma2/python/Plugins/Extensions/Aglare" ]; then
    cp -r "$TMPPATH/NssWidget-main/usr/lib/enigma2/python/Plugins/Extensions/Aglare"/* "$PLUGINPATH/" 2>/dev/null
    echo "✅ Copied from standard plugin directory"
elif [ -d "$TMPPATH/NssWidget-main/usr/lib64/enigma2/python/Plugins/Extensions/Aglare" ]; then
    cp -r "$TMPPATH/NssWidget-main/usr/lib64/enigma2/python/Plugins/Extensions/Aglare"/* "$PLUGINPATH/" 2>/dev/null
    echo "✅ Copied from lib64 plugin directory"
elif [ -d "$TMPPATH/NssWidget-main/usr" ]; then
    # Copy entire usr tree
    cp -r "$TMPPATH/NssWidget-main/usr"/* /usr/ 2>/dev/null
